...
TBD — A dev-only Dex OIDC provider in the local Docker Compose stack so the real OIDC login path (authorization code + PKCE, id_token verification, callback provisioning) can be exercised offline with static test accounts, with no Google credentials and no app-code differences between Dex (dev) and Google (prod).
