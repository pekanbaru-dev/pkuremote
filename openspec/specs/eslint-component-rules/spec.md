# eslint-component-rules Specification

## Purpose

TBD - created by archiving change enforce-component-library. Update Purpose after archive.
...
A Vitest test at `src/lib/eslint-config-tests/eslint-component-rules.test.ts` SHALL use the ESLint Node API to lint an inline fixture string under two synthetic file paths: one under `src/routes/whatever/+page.svelte` and one under `src/lib/components/ui/whatever.svelte`. The test SHALL assert that the rule fires on the route path and does NOT fire on the component path. The test SHALL also assert the rule's message includes the canonical `$lib/components/ui` reference. (The test uses an inline `testConfig` array passed via `overrideConfig`, bypassing the project's `projectService: true` config that rejects synthetic file paths. The `testConfig` mirrors the project's actual rule config so the test is a faithful exercise of the real rules.)

#### Scenario: Vitest test verifies rule fires on a route

- **WHEN** `pnpm test:unit -- --run` runs
- **THEN** the new test at `src/lib/eslint-config-tests/eslint-component-rules.test.ts` passes, confirming that the rule fires on raw `<input>` in a `src/routes/` fixture
