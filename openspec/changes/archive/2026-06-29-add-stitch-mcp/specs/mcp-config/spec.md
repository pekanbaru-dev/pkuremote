# mcp-config Specification (delta)

## Purpose

Defines the cross-tool convention for registering MCP servers across the project's four agent-tool configuration files (opencode, .agents, Claude Code, Codex). Covers the secret-management rule, the per-platform header syntax, the four-file sync rule, and the onboarding step in AGENTS.md.
