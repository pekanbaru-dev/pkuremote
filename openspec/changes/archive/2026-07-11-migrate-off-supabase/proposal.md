...
- `drizzle-integration`: `.env.example` and the db scripts drop all Supabase variables and `DIRECT_URL` in favor of a single `DATABASE_URL`; the schema adds app-owned `users`, `oauth_accounts`, and `sessions` tables (with case-insensitive email uniqueness), re-points `profiles.id` at `users.id`, removes the `profiles` RLS policies, and the seed provisions users without the Supabase admin API. The Supabase-coupled migration history (FK to `auth.users`, RLS, `handle_new_user`) is **baselined/squashed** so a fresh `pnpm db:migrate` applies cleanly to an empty self-hosted Postgres.
- `production-deploy`: `docker-compose.prod.yml` gains an in-stack `postgres` service + `postgres_data` volume (the database is no longer external Supabase), the `app` service swaps Supabase env for `DATABASE_URL` (→ the `postgres` service) + `OIDC_*`, and the prod `.env.example`/README are updated accordingly.
- `docker-dev-loop`: `docker-compose.yml` gains an in-stack `postgres` and a dev-only `dex` service, drops Supabase env, and documents the `OIDC_ISSUER` topology so the issuer resolves identically from the browser and the app (host-run vs containerized).
...
- **Schema/DB:** new `users`, `oauth_accounts`, `sessions` tables; `db/auth-ref.ts` (the `auth.users` shim) removed; `profiles` FK re-pointed; RLS + `handle_new_user` removed. Migration history is **baselined** (the existing `0000`/`0001` reference `auth.users`/`auth.uid()`/the `authenticated` role and cannot apply to plain Postgres) so a fresh migrate succeeds on an empty Docker Postgres; greenfield (no data migration).
- **Infra/config:** `docker-compose.yml` (dev) gains an in-stack `postgres` + a dev-only `dex` service + `dex-config.yaml`; `docker-compose.prod.yml`/`deploy.yml` gain an in-stack `postgres` service + `postgres_data` volume, drop Supabase env, and gain `OIDC_*` + `DATABASE_URL`. `.env.example`, `DEPLOY.md`, and `LOCAL_DEV_ADMIN.md` updated (incl. the dev `OIDC_ISSUER` topology). Google Cloud Console OAuth client re-registered with the app's own callback (prod) — no Supabase redirect URL.
