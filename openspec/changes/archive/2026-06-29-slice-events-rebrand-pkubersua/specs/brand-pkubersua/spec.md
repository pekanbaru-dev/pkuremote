# brand-pkubersua Specification (delta)

## Purpose

Defines the brand identity for "PKUBersua" — the rename from "PKU Remote", the Stitch-based golden palette adopted as canonical, the Hanken Grotesk + Manrope typography swap, the public surface URL `pkubersua.com`, the booking email `hello@pkubersua.com`, and the updated product register copy in `PRODUCT.md` and `DESIGN.md`.
