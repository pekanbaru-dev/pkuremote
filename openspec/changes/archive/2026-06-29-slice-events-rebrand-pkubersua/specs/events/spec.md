...
Defines the PKUBersua event system — the homepage event listing, the event detail page, the `Event` data shape, the dummy-data service, the `features/events/` component folder, and the booking flow (mailto-based, no backend registration).
...
Each `EventCard` (in `src/lib/features/events/components/event-card.svelte`) SHALL render the event's `bannerUrl` (or a 16:9 placeholder block when null), `title` (Spectral/display font, 1.25rem, max 2 lines truncated), `startsAt` formatted as a localized date (e.g., "Kamis, 11 Jul 2026 · 18:30 WIB"), `location`, a `StatusBadge` with the variant matching the event status (primary for upcoming, neutral for past), and a price summary (free / normal / promo with strikethrough). The whole card SHALL be a single `<a>` linking to `/events/{slug}` with the full title as accessible name.

#### Scenario: A reader scans the event listing

- **WHEN** a visitor sees an `EventCard` in the listing
- **THEN** they can read the event title, the date in their locale, the location, the status at a glance, and whether the event is free / paid / promo — without clicking into the detail page.

#### Scenario: A keyboard user tabs through the listing

- **WHEN** a keyboard user focuses an `EventCard`
- **THEN** the focus ring is visible (using the `--color-ring` token, mapped to primary), the card is reachable via a single Tab stop, and pressing Enter navigates to the event detail page.
