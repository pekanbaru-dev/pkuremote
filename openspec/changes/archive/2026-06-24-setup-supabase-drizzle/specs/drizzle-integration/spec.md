...
- `profiles` — one row per `auth.users` row, with a foreign key to `auth.users.id` and a `display_name` column.
...
Each table SHALL declare a primary key (`id` as `uuid` with a `gen_random_uuid()` default) and SHALL have Row Level Security **disabled** in this change (the first auth-gated feature will add the matching policies).
...
#### Scenario: RLS is off so the service role can read and write

- **WHEN** the seed script inserts rows using the service role connection
- **THEN** the inserts succeed and the rows are visible in Drizzle Studio
