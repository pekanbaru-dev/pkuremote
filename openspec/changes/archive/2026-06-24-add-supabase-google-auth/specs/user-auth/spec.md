...
TBD — Single sign-in surface for the SvelteKit FE, backed by Supabase Auth's Google OAuth provider. Provides session validation in `hooks.server.ts`, automatic provisioning of a `profiles` row on first sign-in, a `/login` page with one CTA, a `/myprofile` page that requires a session, and a sign-out action. Closes issues #5 and #6.
