...
<!-- Preview banner -->
<div
	class="sticky top-0 z-50 flex items-center justify-between gap-4 border-b border-hairline bg-primary/10 px-4 py-2.5 text-label-sm text-primary"
>
...
<div
	class="relative mx-auto w-full max-w-7xl px-2 pt-2 pb-4 tablet:px-[0.8rem] tablet:pt-2 tablet:pb-[1.4rem] min-h-screen"
>
			...
			<header class="mb-8">
				...
				{#if article.coverImageUrl}
					<div class="mb-4 rounded-2xl overflow-hidden border border-hairline">
						<img src={article.coverImageUrl} alt="" class="w-full object-cover max-h-80" />
					</div>
				{/if}
