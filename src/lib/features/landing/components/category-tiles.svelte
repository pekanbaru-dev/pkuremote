<script lang="ts">
	...
	function linkClass(tone: string) {
		return tone === "emerald"
			? `${BASE_LINK} border-b-2 border-b-[#073d3d] bg-[#f7fbfa]`
			: `${BASE_LINK} hover:bg-[#fffdf5]`;
	}

	function iconClass(tone: string) {
		return `${BASE_ICON} ${categoryStyles[tone]}`;
	}
</script>
...
>
	{#each categories as [label, Icon, tone] (label)}
		<a href="#events" class={linkClass(tone)}>
			<span class={iconClass(tone)}><Icon size={20} /></span>
			{label}
		</a>
	{/each}
</div>
