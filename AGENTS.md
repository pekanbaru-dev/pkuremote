...
- **Typography:** Plus Jakarta Sans is the single family for display, body, and label roles, loaded at weights 400–800 from Google Fonts via the `<link>` in `src/app.html`. Keep the semantic tokens `--font-display`, `--font-body`, and `--font-label`, with all three mapped to Plus Jakarta Sans. Hanken Grotesk, Manrope, Spectral, and Source Sans 3 are retired.
