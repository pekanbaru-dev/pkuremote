...
# Read at runtime by $lib/server/storage. Without these the admin upload and
# presigned-URL paths fail. They must ALSO be passed through in
# docker-compose.deploy.yml — a var in .env that the compose file does not
# forward never reaches the container.
...
| Deploy red at "Deploy over SSH": `No such image: …:sha-<commit>`, and `compose pull` said `Skipped` | the server's `docker-compose.deploy.yml` has `pull_policy: never` (repo says `always`). The deploy passes an immutable `sha-` tag the box has never seen, so it must be fetched. Fix the line on the server, or re-copy the repo file. The old container keeps serving throughout, so the site looks fine — only Actions goes red |
| Server compose drifted from the repo                                                                | it is copied by hand (no sync), so edits made on the box (e.g. adding `R2_*`) never come back. Diff before re-copying, or you will silently drop them                                                                                                                                                                             |
